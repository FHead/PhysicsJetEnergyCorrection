...
int main(int argc, char *argv[])
{
   ...
   string Function = GetFunction("t");
   
   out << "{1 JetEta 4 Rho JetPt JetA JetPhi ";
   out << "max(0.0001,1-(z/y)*(" << Function << "))";
   ...
   for(int i = 0; i + 1 < (int)EtaBins.size(); i++)
   {
      ...
      out << EtaBins[i] << " " << EtaBins[i+1] << " " << 8 + Result.size();
...
vector<double> FitClass::HEMDoFit(bool FixOutside, double FixP0, double FixP1, double FixP2)
{
   ...
   Worker->SetLimitedVariable(3, "HEMP0", 0.0, 0.1, -100, 100);
   Worker->SetLimitedVariable(4, "HEMP1", 1.0, 0.1, -100, 100);
   Worker->SetLimitedVariable(5, "HEMP2", 1.0, 0.1, -100, 100);
   Worker->SetLimitedVariable(6, "HEMP3", -2e-3, 0.1, -100, 100);
...
double HEMFunction(const double *Parameters, double Rho, double PT, double Phi)
{
   ...
   // Modified 22675 - part 2
   double OOAIn = Parameters[3] + Parameters[4] * Rho + Parameters[5] * PT + Parameters[6] * PT * PT;
