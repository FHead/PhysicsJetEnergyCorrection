...
int FindParameterCount(string F);
string ModifyFormula(string F1, string F2);
int CountParameters(string FileName);
string ToString(int Number);
string GetG(CommandLine &CL, char T, int Index);
...
void SplitUp(vector<Record> &R1, vector<Record> &R2, double MinEta, double MaxEta, double KillShift);
void GenerateTextFile(string FileName, string Header, vector<Record> R1, vector<Record> R2);
int FindBin(vector<double> &Bins, double X);
void PTClipping(vector<Record> &Records, double PTClip);
void RelaxRange(vector<Record> &Records, string Formula);
...
int main(int argc, char *argv[])
{
   ...
   string NewFormula2 = ModifyFormula(Formula1, Formula2);

   cout << "Modified formula2 = " << NewFormula2 << endl;

   int ParameterCount = CountParameters(OutsideFileName);
   char T = 'x';
   if(ParameterCount == 1)   T = 'y';
   if(ParameterCount == 2)   T = 'z';
   if(ParameterCount == 3)   T = 't';

   string G = GetG(CL, T, FindParameterCount(NewFormula2));

   string NewFormula = Formula1 + "-" + "(" + NewFormula2 + "-" + Formula1 + ")/" + C + "*" + G;
   ...
   PTClipping(LinesOutside, 8);
   PTClipping(LinesInside, 8);

   RelaxRange(LinesInside, Formula2);
   ...
   double HEMShift = CL.GetDouble("HEMShift", 0.0);
   
   SplitUp(LinesInside, LinesOutside, MinEta - R0, MaxEta + R0, (XR - XL - HEMShift * 2) / 2);
   ...
   //    cout << i << " " << LinesInside[i].BinMin[0] << " " << LinesInside[i].BinMax[0]
   //       << " " << LinesInside[i].BinMin[1] << " " << LinesInside[i].BinMax[1]
   //       << endl;
   // for(int i = 0; i < (int)LinesOutside.size(); i++)
   //    cout << i << " " << LinesOutside[i].BinMin[0] << " " << LinesOutside[i].BinMax[0]
   //       << " " << LinesOutside[i].BinMin[1] << " " << LinesOutside[i].BinMax[1]
   //       << endl;
...
double GetAverage(CommandLine &CL)
{
   ...
   double Shift = CL.GetDouble("Shift", 0.0);
   ...
   HEMPhiMin = HEMPhiMin + HEMShift;
   HEMPhiMax = HEMPhiMax - HEMShift;
...
int FindParameterCount(string F)
{
   ...
   for(int i = 0; i < F.size(); i++)
   {
      ...
      int NFormula = atoi(F.substr(i + 1, j - i - 1).c_str());
      if(NFormula + 1 > N)
         N = NFormula + 1;
   }
...
string ModifyFormula(string F1, string F2)
{
   ...
   // Find max paramter in F1
   int N = FindParameterCount(F1);
...
string GetG(CommandLine &CL, char T, int Index)
{
   ...
   double XR = CL.GetDouble("HEMPhiMax");
   double R0 = CL.GetDouble("R0");
   double Shift = CL.GetDouble("HEMShift", 0.0);
   XL = XL + Shift;
   XR = XR - Shift;
   ...
   if(XR > 0)   sprintf(R1, "(%.4f-%c+[%d])/%.4f", XL, T, Index, R0 / sqrt(2));
   else         sprintf(R1, "(%.4f-%c+[%d])/%.4f", XL, T, Index, R0 / sqrt(2));
   sprintf(G1, "(TMath::Erf(%s)+1)/2", R1);
   if(XL > 0)   sprintf(R2, "(%c-%.4f+[%d])/%.4f", T, XR, Index, R0 / sqrt(2));
   else         sprintf(R2, "(%c+%.4f+[%d])/%.4f", T, -XR, Index, R0 / sqrt(2));
   sprintf(G2, "(TMath::Erf(%s)+1)/2", R2);
...
void SplitUp(vector<Record> &R1, vector<Record> &R2, double MinEta, double MaxEta, double KillShift)
{
   ...
   Assert(R1[0].NBin == 1 || R1[0].NBin == 2, "More than two bin variables not supported yet.");

   vector<double> Bins;
   vector<vector<double>> BinsY;

   if(R1[0].NBin >= 1)
   {
      for(int i = 0; i < (int)R1.size(); i++)
      {
         if(R1[i].BinMin[0] > MaxEta || R1[i].BinMax[0] < MinEta)
            R1[i].TheRest.push_back(KillShift);
         else
            R1[i].TheRest.push_back(0);
      }

      vector<double> NoInside(R1[0].TheRest);
      R1[0].TheRest.back() = 0.24905;
      ...
      {
         if(R2[i].BinMin[0] > MaxEta || R2[i].BinMax[0] < MinEta)
         {
            Record X = R2[i];
            X.TheRest = NoInside;
            R1.push_back(X);
   ...
   if(R1[0].NBin >= 2)
   {
      BinsY.resize(Bins.size(), vector<double>());

      int iX = 0;
      for(int i = 0; i < (int)R1.size(); i++)
      {
         double X = (R1[i].BinMin[0] + R1[i].BinMax[0]) / 2;
         if(X >= Bins[iX+1])
            iX = iX + 1;
         BinsY[iX].push_back(R1[i].BinMin[1]);
         BinsY[iX].push_back(R1[i].BinMax[1]);
      }
      iX = 0;
      for(int i = 0; i < (int)R2.size(); i++)
      {
         double X = (R2[i].BinMin[0] + R2[i].BinMax[0]) / 2;
         if(X >= Bins[iX+1])
            iX = iX + 1;
         BinsY[iX].push_back(R2[i].BinMin[1]);
         BinsY[iX].push_back(R2[i].BinMax[1]);
      }

      cout << BinsY[0].size() << endl;

      for(vector<double> &B : BinsY)
      {
         sort(B.begin(), B.end());
         auto iter = unique(B.begin(), B.end(), [](double l, double r) {return fabs(l - r) < Tolerance;});
         B.erase(iter, B.end());
      }
      ...
      for(int i = 0; i < (int)R1.size(); i++)
      {
         ...
         if(R1[i].BinMin[1] >= BinsY[iX][iY+1])
            iY = iY + 1;

         if(R1[i].BinMax[1] > BinsY[iX][iY+1] + Tolerance)
         {
            Record NewRecord = R1[i];
            R1[i].BinMax[1] = BinsY[iX][iY+1];
            NewRecord.BinMin[1] = BinsY[iX][iY+1];
            R1.insert(R1.begin() + i + 1, NewRecord);
         }
      }
      ...
      for(int i = 0; i < (int)R2.size(); i++)
      {
         ...
         if(R2[i].BinMax[1] > BinsY[iX][iY+1] + Tolerance)
         {
            Record NewRecord = R2[i];
            R2[i].BinMax[1] = BinsY[iX][iY+1];
            NewRecord.BinMin[1] = BinsY[iX][iY+1];
            R2.insert(R2.begin() + i + 1, NewRecord);
         }
      }
...
void GenerateTextFile(string FileName, string Header, vector<Record> R1, vector<Record> R2)
{
   ...
   for(int i = 0; i < (int)R1.size(); i++)
   {
      if(R1[i].NBin == 2 && R1[i].BinMax[1] > 6499)
         if(R1[i].ParMax[0] > R2[i].ParMax[0])
            R1[i].ParMax[0] = R2[i].ParMax[0];
   ...
   for(int i = 0; i < (int)Records.size(); i++)
   {
      if(Records[i].BinMin.size() == 1 || Records[i].BinMin[1] < 0.002)   // game on!
      {
         if(Records[i].BinMin.size() > 1 && Records[i].BinMax[1] < PTClip)
         {
            ...
            Records[i+1].BinMin[1] = 0.001;

            Records.erase(Records.begin() + i);
            i = i - 1;
         }
         else
            Records[i].ParMin[0] = PTClip;
      }
      else
      {
         if(Records[i].BinMax[1] < 6499)
         {
            Records[i].ParMin[0] = Records[i].BinMin[1];
            Records[i].ParMax[0] = Records[i].BinMax[1];
         }
         else
            Records[i].ParMin[0] = Records[i].BinMin[1];
      }
...
void RelaxRange(vector<Record> &Records, string Formula)
{
   TF1 F("F", Formula.c_str(), 0, 6500);
   
   for(int i = 0; i < (int)Records.size(); i++)
   {
      if(Records[i].ParMax[0] < Records[i].BinMax[1] - Tolerance)
      {
         for(int j = 0; j < (int)Records[i].TheRest.size(); j++)
            F.SetParameter(j, Records[i].TheRest[j]);
      
         double JEC = F.Eval(Records[i].ParMax[0]);

         Record NewRecord = Records[i];
         NewRecord.ParMin[0] = Records[i].ParMax[0];
         NewRecord.ParMax[0] = Records[i].BinMax[1];
         NewRecord.BinMin[1] = Records[i].ParMax[0];
         NewRecord.BinMax[1] = Records[i].BinMax[1];

         Records[i].BinMax[1] = Records[i].ParMax[0];
         
         NewRecord.TheRest[0] = JEC;
         for(int j = 1; j < (int)Records[i].TheRest.size(); j++)
            NewRecord.TheRest[j] = 0;

         Records.insert(Records.begin() + i, NewRecord);
      }
   }
}
