...
string GetG(CommandLine &CL)
{
   ...
   if(XR > 0)   sprintf(R1, "(%.4f-t)/%.4f", XL, R0 / sqrt(2));
   else         sprintf(R1, "(%.4f-t)/%.4f", XL, R0 / sqrt(2));
   sprintf(G1, "(TMath::Erf(%s)+1)/2", R1);
   if(XL > 0)   sprintf(R2, "(t-%.4f)/%.4f", XR, R0 / sqrt(2));
   else         sprintf(R2, "(t+%.4f)/%.4f", -XR, R0 / sqrt(2));
